...
\begin{document}
	...
	%%% ACF Espero que o acarajé não tenha ficado indigesto
	...
	%%% ACF \todo[inline]{Melhor na forma de algoritmo, com o "patch" como entrada, e o ponto no plano HxC como saída.}
	%%% ETCC metodologia modificada
	...
	\begin{figure}[hbt]
		...
		\label{fig:WATG}
	\end{figure}
	%%% ACF "Patch" ao invés de "Texture"
	%%% ACF "Vector of observations"
	%%% ACF "Feature in the HxC plane" (último bloco)
	%%% ETCC Done
	...
	%%% ACF O conceito é "fuzzy", mas eu concordo (nebulosamente); os autores afirmam isso.
	...
	%%% ACF Poderia ser MxN, para ficar mais geral, desde que M não seja usado mais adiante. Tornamos a técnica específica ao falar da Hillbert-Peano.
	%%% ETCC Definição modificada
	...
	\end{mydef}
	%%% ACF Tirei o \times de M N. Repare que você usou esse operador para denotar o produto cartesiano. Iria ficar confuso usá-lo com outro significado.
	...
	%%% ACF Não podemos voltar ao "tradicional". O texto tem que apresentar primeiro o tradicional, e depois a contribuição
	%%% ETCC Modificado
	...
	%%% ACF Dar uma boa referência para as cotas do HxC
	%%% ETCC Referência adicionada
	...
	%%% ACF Eu acho melhor não agrupar as figuras, e produzir todas em PDF
	%%% ETCC Figuras alteradas	
	\begin{figure*}[hbt]
		\begin{tabular}{cccc}
	...
	Thus, the following results demonstrate the KNN model with $k = 20$.
	...
	%%% ACF Não ficou claro; finalmente, k=20? Não precisa entrar em tantos detalhes.
	%%% ETCC Modifiquei a frase
