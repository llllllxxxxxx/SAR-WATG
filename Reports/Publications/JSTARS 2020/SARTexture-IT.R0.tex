...
\begin{document}
	...
	Recent works propose different weightings in the calculation of relative frequencies for ordinal patterns with different amplitude variances, making them contribute differently to the final value of the permutation entropy (PE) and, thus, incorporating amplitude change information.
	In this work, for comparison, we will use the following methods as a baseline.
	
	\subsection{Weighted-permutation entropy}
	
	Weighted-permutation entropy (WPE) was proposed by~\cite{Fadlallah2013Weightedpermutation}. 
	Since the traditional Bandt-Pompe method neglects the amplitude differences between the ordinal patterns, the main motivation of this algorithm is to save useful amplitude information carried by the signal in each produced pattern.
	
	Therefore, let $X_t^{(D, \tau)}$ be the arithmetic mean:
	\begin{equation}
	\bar X_t^{(D, \tau)} = \frac{1}{D} \sum_{k = 1}^{D} x_{t + (k - 1)}
	\end{equation}
	We have that the weight $w_{t}$ will be the variation of each vector $X_t^{(D, \tau)}$ in relation to the average $\bar X_t^{(D, \tau)}$:
	\begin{equation}
	w_{t} = \frac{1}{D} \sum_{k = 1}^{D}[x_{t + (k - 1)} - \bar X_t^{(D, \tau)}]^2
	\end{equation}
	Finally, the probability distribution is given from the weighted relative frequencies:
	\begin{equation}
	...
	Fine-grained permutation entropy (FGPE) was introduced in~\cite{xiao2009fine}, consisting of more one approach to the problem of not considering amplitude information in the standard Bandt-Pompe algorithm. 
	...
	\begin{equation}
	\beta_t = \{|x_{t+1} - x_t|, \dots, |x_{t+(D-1)} - x_{t+(D-2)}|\}
	\end{equation}
	The $w_t$ factor is calculated so that it is a quantifier of the differences $\beta_t$:
	\begin{equation}
	w_t = \left \lfloor \frac{\max(\beta_t)}{std(\beta_t) \times \alpha} \right \rfloor
	\end{equation}
	where $\alpha$ is another user-defined parameter and $\lfloor \rfloor$ is the floor function that holds the entire part of the operand.
	The $\beta_t$ is then added as a symbol at the end of the corresponding pattern, causing $\Pi$ to need to be updated dynamically when a new pattern is found.
	\begin{equation}
	...
	The Amplitude-aware permutation entropy (AAPE) method was proposed in~\cite{azami2016amplitude}.
	Such a method consists of weighting the amplitude of ordinal patterns by quantifying the mean and the differences of the elements.
