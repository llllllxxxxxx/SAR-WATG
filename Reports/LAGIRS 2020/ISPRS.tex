...
\begin{document}
	...
	\todo[inline]{Ainda não gostei dos parágrafos acima, vê se o texto em vermelho ficou melhor}
	...
	This curve scans an array of pixels of dimension $2^k \times 2^k$, where $k \in \mathbb{N}$, never keeping the same direction for more than three consecutive points, so that each pixel in a grid is traversed only once, as shown in Figure~\ref{fig:Hilbert}.
	...
	%%%\todo[inline]{a letra $m$ foi utilizada para designar a dimensão dos padrões ordinais nas seções seguintes. O que ela está desginando aqui? Precisa corrigir isso.} (RESOLVIDO)
	...
	As we can see in Figure~\ref{fig:D3T1}, when we apply small delay and dimension values (the best result was obtained when $m = 3$ and $\tau = 1$) with WATG, in urban region data, we obtained a probability distribution with few extreme values, representing the transitions of the maximum values of the series.
