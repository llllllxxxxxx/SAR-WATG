...
\address{
	...
	\textsuperscript{3 }Instituto de F\'isica, Universidade Federal de Alagoas, Brasil \\ oarosso@if.ufal.br, and\\
...
Let $\mathbb{P} = \{p_{(\pi_1, \pi_1)}, p_{(\pi_1, \pi_2)}, \dots, p_{(\pi_{D!}, \Pi_{D!})} \}$ be the probability distribution taken from the time series weighted amplitude transition graph $\mathbb{X}$.
The Shannon entropy is given by:	
\begin{equation}
H(\mathbb{P}) = -\sum_{i=1}^{D!D!} p_i \log p_i .
\label{eq:Entropia}
\end{equation}
...
\begin{equation}
D(\mathbb{P}, \mathbb{U}) = \sum_{i=1}^{D!D!} \Big(p_i \log\frac{p_i}{u_i} +
