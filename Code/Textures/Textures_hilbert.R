...
# Textures_hilbert.R 
#
# Analysis of the brodatz textures using Hilbert space filling curves
#
...
Textures.Hilbert.Analysis <- function(Analysis){
  ...
  if(Analysis == 1){ #Analysis of textures characterization
        for(i in 1:(length(n)*length(tal))){
          ...
          for(j in 1:ntex){
            ...
            probability[j, ] <- Bandt.Pompe(get.serie, n[a], dim(get.serie)[1])
          }
        ...
        ggarrange(plots[[1]], plots[[2]], plots[[3]], plots[[4]], plots[[5]],
          ...
          ggtitle(expression(italic("Textures Hilbert Curve Analysis"))) + xlab(expression(italic(H))) + ylab(expression(italic(C))) + theme_igray() +
        ...
        ggarrange(plots[[1]], plots[[2]], plots[[3]], plots[[4]], plots[[5]],
                  ...
                  plots[[21]], plots[[22]], plots[[23]], plots[[24]], plots[[25]],
                  ncol=5, nrow=5, common.legend = TRUE, legend = "right") + 
          ggtitle(expression(italic("Textures with and without contrast - Hilbert Curve Analysis"))) + xlab(expression(italic(H))) + ylab(expression(italic(C))) + theme_igray() +
          theme(text=element_text(size=14, family="Times New Roman"), axis.text.x=element_blank(), axis.text.y=element_blank(),plot.title = element_text(hjust=0.5)) + 
          guides(colour = guide_legend(override.aes = list(size=2)))
