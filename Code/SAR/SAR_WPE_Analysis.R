...
# SAR_WPE_Analysis.R
#
# SAR image analysis using Bandt-Pompe weight for symbolization of the patterns 
#
...
SAR.WPE.HC.Analysis <- function(){
  ...
  for(i in 1:(length(n)*length(tal))){
    ...
    for(j in c(1:ns.guatemala)){
      ...
      ts = img[hilbertcurve]
      probability[j, 1:factorial(n[a])] <- WPE(ts, n[a], tal[b])
    }
    cat("Guatemala\n")
    ...
    for(j in c(1:ns.canaveral.behavior1)){
      ...
      ts = img[hilbertcurve]
      probability[ns.guatemala + j, 1:factorial(n[a])] <- WPE(ts, n[a], tal[b])
    }
    cat("Cape 1\n")
    ...
    for(j in c(1:ns.canaveral.behavior2)){
      ...
      ts = img[hilbertcurve]
      probability[(ns.canaveral.behavior1 + ns.guatemala) + j, 1:factorial(n[a])] <- WPE(ts, n[a], tal[b])
    }
    cat("Cape 2\n")
    ...
    for(j in c(1:ns.munich)){
      ...
      ts = img[hilbertcurve]
      probability[(ns.canaveral.behavior1 + ns.canaveral.behavior2 + ns.guatemala) + j, 1:factorial(n[a])] <- WPE(ts, n[a], tal[b])
    }
    cat("Munich\n")
    ...
    Entropy.Complexity.Fisher <- Entropy.Complexity.Fisher.Points(probability, dim(probability)[1], n[a])
    
    file = paste("SAR.Entropy.Complexity.Fisher", "D", n[a], "t", tal[b], ".csv", sep = "")
    write.csv(Entropy.Complexity.Fisher,file)
  ...
  ggarrange(plots[[1]], plots[[2]], plots[[3]], plots[[4]], plots[[5]],
    ...
    ggtitle(expression(italic("SAR Images - WPE Analysis"))) +
