...
# RMS_M_Analysis.R
#
# SAR image analysis using RMS and angular coefficient of linear regression
#
...
RMS.M.Analysis.Classes <- function(color.signal, shape.signal, signal.values){
  ...
  signal.values <- data.frame("RSM" = signal.values$RSM, "M" = signal.values$M, "Color" = Color, "Shape" = Shape, "Texture" = Texture)
  
  p = ggplot(signal.values, aes(x = signal.values$M, y = signal.values$RSM)) +
...
SAR.RMS.M.Analysis <- function(){
  
  hilbertcurve <- unlist(read.table("../../Data/Hilbert/HilbertCurves128.txt")) + 1
  RSM = M = pearson  = color = shape = rep(0, (ns.guatemala + ns.canaveral.behavior1 + ns.canaveral.behavior2 + ns.munich))
  ...
  for(j in c(1:ns.canaveral.behavior1)){
    ...
    y = hist.default(ts)$counts + 0.5
    lm.model <- lm(log(y) ~ log(x))
    M[ns.guatemala + j] = lm.model$coefficients[2]
  ...
  for(j in c(1:ns.canaveral.behavior2)){
    ...
    y = hist.default(ts)$counts + 0.5
    lm.model <- lm(log(y) ~ log(x))
    M[(ns.canaveral.behavior1 + ns.guatemala) + j] = lm.model$coefficients[2]
  ...
  signal.values <- data.frame(RSM, M)
  
  p <- RMS.M.Analysis.Classes(color, shape, signal.values) + theme(axis.title.x=element_blank(),
                                                                   axis.title.y=element_blank(), 
                                                                   legend.position = "none")
  plot(p) + ggtitle(expression(italic("SAR Images - M and RMS"))) +
    xlab(expression(italic(M))) + ylab(expression(italic(RMS))) + labs(colour=expression(italic(Regions))) +
