...
# GammaEstimator.R 
#
# Estimation of gamma distributions parameters from different regions obtained by SAR image textures
#
...
Gamma.Estimation.Regions <- function(option){
  ...
  if(option == 1){ #Forest regions in Guatemala
    ...
    for(j in c(1:ns.guatemala)){
      ...
      gamma.parameters <- fitdistr(z[z>0], "gamma")
...
file.guatemala = paste("gamma.guatemala",".csv", sep = "")
write.csv(gamma.guatemala,file.guatemala)

file.cape1 = paste("gamma.cape1",".csv", sep = "")
write.csv(gamma.cape1,file.cape1)

file.cape2 = paste("gamma.cape2",".csv", sep = "")
write.csv(gamma.cape2,file.cape2)

file.munich = paste("gamma.munich",".csv", sep = "")
write.csv(gamma.munich,file.munich)
